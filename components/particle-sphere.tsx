...
function generateSphereGeometry(count = 25000, radius = 1.5) {
  const positions = new Float32Array(count * 3);
  const sizes = new Float32Array(count);
  const core = new Float32Array(count);

  for (let i = 0; i < count; i++) {
    const r = radius * Math.pow(Math.random(), 1.6);
    ...
    core[i] = r < radius * 0.3 ? 1.0 : 0.0;
  }

  return { positions, sizes, core, count };
}
...
function DotSphere({ autoRotate = false, particleSize = 0.12, mouse, groupRef }) {
  ...
  useFrame((state) => {
    ...
    shaderRef.current.uniforms.uMouse.value = mouse.smoothed
    ...
    if (groupRef.current) {
      parallaxTarget.current.x = mouse.smoothed.x * 0.2
      parallaxTarget.current.y = mouse.smoothed.y * 0.2
      parallaxTarget.current.z = Math.sin(mouse.smoothed.x * Math.PI) * 0.15
      ...
      const targetRotX = mouse.smoothed.y * 0.1
      const targetRotY = mouse.smoothed.x * 0.1
    ...
    mouse.smoothed.lerp(mouse.current, 0.05)
  })
...
export default function ParticleSphere({ autoRotate }) {
  const mouse = useRef({
    ...
    smoothed: new Vector2(0, 0)
  })
  const particleSize = 0.12

  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      mouse.current.current.x = (e.clientX / window.innerWidth) * 2 - 1
      mouse.current.current.y = -(e.clientY / window.innerHeight) * 2 + 1
    }
  ...
  return (
    <Canvas
      ...
      dpr={[1, 2]}
    >
      <Scene autoRotate={autoRotate} particleSize={particleSize} mouse={mouse.current} />
    </Canvas>
  )
}
